...
const checkId = generalFunctions.checkId,
      stampToJSON = generalFunctions.stampToJSON
;
...
const listThread = (req, res) => {
  ...
  MongoClient.connect(mongoUri, flagObj, (error, db) => {
    ...
    dbo.collection(board).find({}, projection).sort(shortObj).limit(10).toArray((error, result) => {
      if (error) throw error;
...
const reportThread = (req, response) => {
  const board = req.params.board;
  ...
  MongoClient.connect(mongoUri, flagObj, (error, db) => {
    ...
    dbo.collection(board).findOneAndUpdate(...modifyObj, (error, result) => {
      if (error) return response.send(`could not reported ${thread_id} ${error}`);
      if (result.lastErrorObject.updatedExisting) {
        return response.send(`reported`);
      }
      return response.send(
        `could not update: invalid id: ${thread_id}`
      );
    });
    db.close();
  });
...
const deleteThread = (req, response) => {
  const board = req.params.board;
  const thread_id = req.body.thread_id;
  const error = checkId(thread_id);
  if (error) return response.send(error);
